 ...
 *
 */
@Designate(ocd = ConfigTcpCustom.class, factory = true)
@Component(//
		name = "Controller.Api.ModbusTcpCustom", //
		immediate = true, //
		configurationPolicy = ConfigurationPolicy.REQUIRE)
public class ModbusTcpApiCustom extends AbstractOpenemsComponent implements Controller, OpenemsComponent, JsonApi, ModbusSlaveApi {

	public static final int UNIT_ID = 1;
	public static final int DEFAULT_PORT = 502;
	public static final int DEFAULT_MAX_CONCURRENT_CONNECTIONS = 5;

	private final Logger log = LoggerFactory.getLogger(ModbusTcpApiCustom.class);
	...
	private ConfigTcpCustom config = null;

	public ModbusTcpApiCustom() {
		super(//
	...
	@Activate
	void activate(ComponentContext context, ConfigTcpCustom config) throws ModbusException, OpenemsException {
	...
	private final AbstractWorker startApiWorker = new AbstractWorker() {
		...
		protected void forever() {
			int port = ModbusTcpApiCustom.this.config.port();
			if (this.slave == null) {
				try {
					// start new server
					this.slave = ModbusSlaveFactory.createTCPSlave(port,
							ModbusTcpApiCustom.this.config.maxConcurrentConnections());
					slave.addProcessImage(UNIT_ID, ModbusTcpApiCustom.this.processImage);
					slave.open();

					ModbusTcpApiCustom.this.logInfo(this.log, "Modbus/TCP Api started on port [" + port + "] with UnitId ["
							+ ModbusTcpApiCustom.UNIT_ID + "].");
					ModbusTcpApiCustom.this.channel(ChannelId.UNABLE_TO_START).setNextValue(false);
				} catch (ModbusException e) {
					ModbusSlaveFactory.close();
					ModbusTcpApiCustom.this.logError(this.log,
							"Unable to start Modbus/TCP Api on port [" + port + "]: " + e.getMessage());
					ModbusTcpApiCustom.this.channel(ChannelId.UNABLE_TO_START).setNextValue(true);
				}
	 ...
	 */
	private void addRecordToProcessImage(int address, ModbusRecord record, OpenemsComponent component) {
